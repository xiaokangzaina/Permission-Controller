...
## 重要变更：v1.8.2 已移除 Web 面板

从 v1.8.2 开始，本插件不再提供前端 Web 设置面板。
...
安装包名称：astrbot_plugin_permission_controller-v1.8.2.zip

Release 地址：

https://github.com/xiaokangzaina/astrbot_plugin_permission_controller/releases/tag/v1.8.2
...
### 为什么 v1.8.2 没有 Web 面板？

因为 Web 面板在部分 AstrBot Dashboard 环境下会出现路由不兼容，导致保存失败。v1.8.2 起已移除，改用插件配置页或配置文件维护。
...
- 回退 v1.8.2 的配置分组改动，恢复默认平铺配置，避免部分环境下插件配置失效。
- 修复私聊白名单为空时普通私聊被误放行的问题。
- 精确屏蔽 after_message_sent 终止传播冗余日志，不影响机器人回复消息日志显示。
- 增加加载日志，便于确认权限规则是否生效。
